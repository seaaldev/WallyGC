...
static void
wgc_arena_destroy(arena_t* arena);
...
static void wgc_arena_destroy(arena_t* arena) { 
  wgc_virtual_unmap((void*)arena, arena->size - 1);
}
...
wgc_arena_alloc(arena_t* arena, size_t bytes) {
  ...
  //TODO: Red Black Tree | A List of Arenas?
...
wgc_arena_expand(arena_t* arena, size_t page_count) {
   ...
   * next to each other get merged. This is 
   * a shit solution! Will get fixed later!
   */ 

...
bool wgc_list_link_valid(struct list_head *new,
                         ...
                         struct list_head *prev)
{
  return (next->prev == prev && prev->next == next && new != next && new !=prev);
}
...
void wgc_list_del_entry(struct list_head *entry) 
{
  ...
  entry->next = (struct list_head *) POISON;
  entry->prev = (struct list_head *) POISON;
}

/* 
 * ------------------------------------------------------------------ 
 * NORMAL ROUTINES BELOW:
 * ------------------------------------------------------------------ 
 */
...
{
  wgc_list_del_entry(entry);
  wgc_list_link(entry, head->next, head);
}
...
{
  wgc_list_del_entry(entry);
  wgc_list_link(entry, head, head->prev);
}
